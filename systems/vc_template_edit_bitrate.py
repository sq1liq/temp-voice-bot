...
class vc_template_edit_br(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @slash_command(name="vc_template_edit_bitrate", description="Edit the bitrate of a voice channel template")
    async def vc_template_edit_br(self, ctx: discord.ApplicationContext, template_name: Option(str, "The name of the template", autocomplete=templates), bitrate: int):
            ...
            if template_name == "all":
                async with db.cursor() as cursor:
                    await cursor.execute("UPDATE vc_templates SET temp_bitrate = ? WHERE guild_id = ?", (bitrate, ctx.guild.id))
                    await db.commit()
                ...
                async with db.cursor() as cursor:
                    await cursor.execute(f"UPDATE vc_templates SET temp_bitrate = ? WHERE guild_id = ? AND template_name = ?", (bitrate, ctx.guild.id, template_name))
                    await db.commit()
            ...
            if template_name == "all":
                em = E(
                    title="<:4928applicationbot:1230570127200882779> | Templates Updated",
...
def setup(bot):
    bot.add_cog(vc_template_edit_br(bot))
