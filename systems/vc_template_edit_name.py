...
async def templates(ctx: discord.AutocompleteContext):
    ...
    await db.close()
    return [template[1] for template in data]



class vc_template_edit_nm(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @slash_command(name="vc_template_edit_name", description="Edit the name of a voice channel template")
    async def vc_template_edit_nm(self, ctx: discord.ApplicationContext, template_name: Option(str, "The name of the template", autocomplete=templates), new_name: str):
        ...
        async with db.cursor() as cursor:
            await cursor.execute("UPDATE vc_templates SET temp_name = ? WHERE guild_id = ? AND template_name = ?", (new_name, ctx.guild.id, template_name))
            await db.commit()
...
def setup(bot):
    bot.add_cog(vc_template_edit_nm(bot))
