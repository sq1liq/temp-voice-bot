...
class hide(commands.Cog):
    ...
    @slash_command(name="vc_hide", description="Hide a temporary voice channel")
    async def vc_hide(self, ctx: discord.ApplicationContext):
                ...
                async with db.cursor() as cursor:
                    ...
                    em = E(
                        title="<:Checken:1230840792382443530> | channel hidden",
                        description="**Your temporary voice channel has been hidden**",
                        color=color
                    )
                    em.set_footer(text=footer, icon_url=footer_icn)
                    em.set_thumbnail(url=thumbnail)
                    ...
                    em = E(
                        title="<:Checken:1230840792382443530> | channel hidden",
                        description=f"**{ctx.author.mention} has hidden their temporary voice channel**",
                        color=color
                    )
                    em.set_footer(text=footer, icon_url=footer_icn)
                    em.set_thumbnail(url=thumbnail)
